...
// Protection : tout utilisateur connecte peut consulter le feed
    ...
    <div class="container">
        <div class="d-flex align-items-start justify-content-between flex-wrap gap-3 mb-4">
            <div>
                ...
                <a href="/gestion_users/view/frontoffice/submit.php" class="btn-submit-link">
        ...
        <div class="container" id="feedContainer">
            ...
            <?php if (empty($devoirs)): ?>
                <div class="empty-state">
                    ...
                    <a href="/gestion_users/view/frontoffice/submit.php" class="btn-submit-link mt-3">
...
<span class="sentiment-badge sentiment-<?= htmlspecialchars($d['sentiment'] ?? 'neutre') ?> <?= ($d['alerte_urgence'] ?? 0) ? 'sentiment-urgent' : '' ?>">
    ...
    ?>
                        ...
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            <a href="/gestion_users/view/frontoffice/submit.php?add_correction_for=<?= $d['id_devoir'] ?>&title=<?= urlencode($d['titre']) ?>" 
    ...
    <i class="fas fa-file-pdf"></i> Exporter PDF
                            ...
                            <a href="/gestion_users/view/frontoffice/submit.php?edit=devoir&id=<?= $d['id_devoir'] ?>" class="btn-edit btn-sm">
        ...
        <i class="fas fa-volume-up"></i>
                        ...
                        <div class="mt-2">
                            <a href="/gestion_users/uploads/devoirs/<?= htmlspecialchars($d['fichier']) ?>"
            ...
            <i class="fas fa-volume-up"></i>
                                    ...
                                    <div class="d-flex align-items-center gap-2 flex-wrap">
                                        ...
                                        <a href="/gestion_users/view/frontoffice/submit.php?edit=correction&id=<?= $c['id_correction'] ?>" class="btn-edit btn-sm">
                                            <i class="fas fa-edit"></i> Modifier
                                        </a>
...
<div class="mt-2">
    <a href="/gestion_users/uploads/correction/<?= htmlspecialchars($c['fichier_corrige']) ?>"
       class="file-link" 
...
function deleteItem(id, type) {
    ...
    fetch('/gestion_users/controller/devoirs.php?action=' + action + '&id=' + id, {
        method: 'GET',
        headers: {
...
function refreshPage() {
    ...
    window.location.href = '/gestion_users/view/frontoffice/feed.php';
}
...
function resolveUrl(url) {
    if (!url) return null;
    let u = url.trim();
    if (u.startsWith('uploads/') || u.startsWith('./uploads/')) u = '/gestion_users/' + u;
...
async function exportDevoirToPDF(devoirCard, devoirId) {
    ...
    for (const sub of correctionSubcards) {
        ...
        if (!fichierUrl) {
            ...
            if (attr) fichierUrl = '/gestion_users/uploads/correction/' + attr;
        }
...
function loadSentimentStats() {
    fetch('/gestion_users/controller/devoirs.php?action=sentimentStats')
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function sendMessage() {
        ...
        // Appeler l'API
        fetch(`/gestion_users/controller/devoirs.php?action=chat&message=${encodeURIComponent(message)}`)
            .then(response => response.json())
            .then(data => {
                hideTyping();
