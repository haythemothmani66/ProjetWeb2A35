...
(() => {
  ...
  const editDevoirModal = new bootstrap.Modal(document.getElementById('editDevoirModal'));
  const editCorrectionModal = new bootstrap.Modal(document.getElementById('editCorrectionModal'));
  ...
  function parseControllerResponse(raw) {
    ...
    if (clean.includes('|')) {
      ...
      return {
        success: normalizedStatus.includes('succ') || normalizedStatus.includes('success'),
  ...
  function renderDevoirTable() {
    ...
    if (!rows.length) {
      elements.devoirsTableBody.innerHTML = '<tr><td colspan="6" class="text-center py-5 text-secondary">No devoirs found.</td></tr>';
      return;
    }
        ...
        <tr>
          ...
          <td class="text-end">
            <div class="d-flex justify-content-end gap-2 flex-wrap">
              ...
              <button class="btn btn-sm btn-white" data-type="devoir" data-action="edit" data-id="${devoir.id_devoir}">Edit</button>
  ...
  function renderCorrectionTable() {
    ...
    if (!rows.length) {
      elements.correctionsTableBody.innerHTML = '<tr><td colspan="6" class="text-center py-5 text-secondary">No corrections found.</td></tr>';
      return;
    }
  ...
  function renderStats() {
    ...
    elements.statTotalCorrections.textContent = String(state.corrections.length);
  }
  ...
  async function loadFeedData() {
    ...
    state.corrections = Array.isArray(correctionPayload.data) ? correctionPayload.data : [];
    renderAll();
  }
  ...
  async function openDevoirDetails(id) {
    const payload = await fetchJson(`${state.controllerUrl}?action=getdevoir&id=${encodeURIComponent(id)}`);
  ...
  async function openCorrectionDetails(id) {
    const payload = await fetchJson(`${state.controllerUrl}?action=getcorrection&id=${encodeURIComponent(id)}`);
    ...
    setDetailsHtml(`Correction #${correction.id_correction}`, [
      ...
      { label: 'Suggestions', value: correction.suggestions_personnalisees || 'N/A' },
      { label: 'Ressources', value: correction.ressources_recommandees || 'N/A' },
  ...
  async function openDevoirEdit(id) {
    ...
    document.getElementById('editDateSoumission').value = devoir.date_soumission || '';
    document.getElementById('editNiveau').value = devoir.niveau_difficulte || 'moyen';
    document.getElementById('editTypeErreur').value = devoir.type_erreur_predominant || 'logique';
    document.getElementById('editTemps').value = devoir.temps_estime_resolution || 1;
    document.getElementById('editProgression').value = devoir.progression_eleve || 0;
  ...
  async function openCorrectionEdit(id) {
    ...
    document.getElementById('editCorrectionId').value = correction.id_correction;
    document.getElementById('editCommentaire').value = correction.commentaire || '';
    document.getElementById('editDateCorrection').value = correction.date_correction || '';
    document.getElementById('editTypeFeedback').value = correction.type_feedback || 'explicatif';
    document.getElementById('editNote').value = correction.note_estimee || 0;
    document.getElementById('editCompetences').value = correction.competences_evaluees || '';
    document.getElementById('editIterations').value = correction.nombre_iterations || 1;
    document.getElementById('editSuggestions').value = correction.suggestions_personnalisees || '';
    document.getElementById('editRessources').value = correction.ressources_recommandees || '';
    document.getElementById('editRapidite').value = correction.rapidite_correction || 1;
    document.getElementById('editTon').value = correction.ton_feedback || 'neutre';
  ...
  async function deleteItem(type, id) {
    ...
    const response = await fetch(`${state.controllerUrl}?action=${action}&id=${encodeURIComponent(id)}`);
    const raw = await response.text();
  ...
  async function saveCorrectionEdit(event) {
    ...
    const { allValid, firstInvalid } = validateForm(elements.editCorrectionForm);

    if (!allValid) {
      showToast('Veuillez corriger les champs invalides de la correction.', 'danger');
    ...
    const response = await fetch(`${state.controllerUrl}?action=updatecorrection`, {
      method: 'POST',
      body: formData,
    });
  ...
  async function handleTableActions(event) {
    const button = event.target.closest('button[data-action][data-id][data-type]');
  ...
  function bindEvents() {
    ...
    document.getElementById('editDevoirModal').addEventListener('hidden.bs.modal', () => {
      resetFormValidation(elements.editDevoirForm);
    });

    document.getElementById('editCorrectionModal').addEventListener('hidden.bs.modal', () => {
      resetFormValidation(elements.editCorrectionForm);
    });
  }
